...
Next.js rebuild of MySkyBuy — same UI, PostgreSQL, search, Razorpay + COD.

## Folder

This app lives in **`C:\Users\dell\Desktop\myskybuy-next`** (separate from the old Express project).

## Setup (local)

```bash
cd C:\Users\dell\Desktop\myskybuy-next
...
- Admin: http://localhost:3000/admin (default `admin` / `myskybuy@123` unless env changed)
...
5. `pm2 start npm --name myskybuy -- start`
